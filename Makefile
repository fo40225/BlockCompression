...
BENCHMARK_INCLUDE=-I $(TOP_DIR)/$(BENCHMARK_DIR) $(BGZF_INCLUDE) $(CPU_INCLUDE) $(LZ4_INCLUDE) $(QUICKLZ_INCLUDE) $(SNAPPY_INCLUDE) $(ZLIB_INCLUDE) $(ZSTD_INCLUDE)
...
CPU_SOURCES=$(CPU_DIR)/CpuSupport.c
CPU_OBJS=$(addprefix obj/,$(notdir $(CPU_SOURCES:.c=.o)))
...
$(SHAREDLIB): $(BGZF_OBJS) $(CPU_OBJS) $(LZ4_OBJS) $(QUICKLZ_OBJS) $(SNAPPY_OBJS) $(ZLIB_OBJS) $(ZSTD_OBJS)
...
obj/%.o : $(CPU_DIR)/%.c
	@mkdir -p $(dir $@)
	@echo "- compiling" $(*F).c
	@$(CC) $(CFLAGS) $(MARCH) $(CPU_INCLUDE) -c $< -o $@
